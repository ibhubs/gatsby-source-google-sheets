...
describe("cleaning rows from GSheets response", () => {
  ...
  it('guesses column types', function () {
    const rows = [{ number: "0", string: "something", null: null, boolean: "TRUE" }, { number: "1", string: "anything", null: null, boolean: "FALSE" }, { number: "2", string: "nothing", null: null, boolean: null }];
    const cleaned = cleanRows(rows);
...
describe("guessing column data type based on all cells in column", () => {
  it('recognizes type for all same cells', function () {
    const rows = [{ numbers: "1,", strings: "something", nulls: null, booleans: "TRUE" }, { numbers: "2", strings: "anything", nulls: null, booleans: "FALSE" }, { numbers: "3,", strings: "nothing", nulls: null, booleans: "TRUE" }];
  ...
  it('ignores null', function () {
    const rows = [{ numbers: "1,", strings: "something", nulls: null, booleans: null }, { numbers: null, strings: "anything", nulls: null, booleans: "FALSE" }, { numbers: "3,", strings: null, nulls: null, booleans: "TRUE" }];
    const guessedTypes = guessColumnsDataTypes(rows);
    expect(guessedTypes.numbers).toBe('number');
    expect(guessedTypes.strings).toBe('string');
    expect(guessedTypes.nulls).toBeUndefined();
  ...
  it('fallbacks to string when types are different', function () {
    const rows = [{ numbers: "1,", strings: "something", nulls: null, booleans: "TRUE" }, { numbers: "2", strings: "anything", nulls: null, booleans: "FALSE" }, { numbers: "3/5,", strings: "nothing", nulls: null, booleans: "TRUE" }];
    const guessedTypes = guessColumnsDataTypes(rows);
